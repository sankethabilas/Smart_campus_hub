...
export default function UserDashboard() {
	...
	const fetchUserData = () => {
		...
		fetch('http://localhost:8085/auth/me', {
			headers: {
	...
	const handleUpdate = async () => {
		// --- FRONTEND VALIDATION (Matches DTO) ---
		const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
		const phoneRegex = /^\d{10}$/;

		if (formData.name.length > 255) {
			setMessage({ type: 'error', text: 'Name must be less than 255 characters' });
			return;
		}
		if (!emailRegex.test(formData.email)) {
			setMessage({ type: 'error', text: 'Email must be valid' });
			return;
		}
		...
		try {
			const response = await fetch(`http://localhost:8085/users/${user.id}`, {
				method: 'PUT',
				headers: {
					'Authorization': `Bearer ${token}`,
					'Content-Type': 'application/json'
			...
			if (response.ok) {
				...
				// Handle backend validation errors if provided
				setMessage({ type: 'error', text: errorData.message || 'Update failed. Check your inputs.' });
			}
		} catch (error) {
